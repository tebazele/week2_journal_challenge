...
Welcome to the CodeWorks Zoo! In this challenge, you will use Javascript to sort zoo animals into pens based on their properties. 

We're going to practice the art of refactoring our code to (1) make it more reusable and (2) to make sure our code is following the single-responsibility principle (each function does only one thing).

By completing this challenge, you will learn more about:
...
## Let's dive in!
## Part 1: Review the initial code
...
#### Go ahead and run bcw serve to see the page on localhost:8080. 
We've already created your zoo layout in the index.html. You'll see that there are pens for birds, mammals, and fish, as well as 5 pens for solitary animals. Note: You may not need to use all of the solitary pens.
...
These functions do what they're supposed to do, but they're all very similar, meaning there's a lot of repeated code. They are also complicated functions that have too many responsibilities. 

Let's take a detailed look at what each function is doing.
...
function penTheBirds() {
    ...
    // NOTE we draw birds to the page by looping through our new birdPen array and storing the emojis for each bird in the 'birdEmojis' variable
    let birdEmojis = '';
    for (let i = 0; i < birdPen.length; i++) {
        const bird = birdPen[i];
        birdEmojis += bird.emoji
    }
    // NOTE we grab the bird pen on the page by its Id
    let birdElem = document.getElementById('birdPen')

    // NOTE we draw the bird emojis into the pen
    birdElem.innerHTML = `<h1>${birdEmojis}</h1>`
}
// NOTE here we call the function that both sorts and draws the birds to the page
penTheBirds()
```
...
```
Our penTheFish function also looks very similar to the other two functions. Let's see if we can fix that by refactoring and minimizing redundant functionality.



## Part 2: Refactoring
##### In the app.js, comment out the following functions before starting to refactor
`penTheBirds()`
`penTheMammals()`
`penTheFish()`

#### Refactor these functions to make one reusable function that will sort all of the animals AND draw them to the page
Let's start with a little refactoring. In your app.js, can you write a reusable sorting function called `penTheAnimals` that takes in two arguments "animalType" and "solitaryPenNumber"? Because not all the types have solitary animals, we'll set a default value of 0 so we only need to send in the `solitaryPenNumber` as an argument when we need it.

#### If you get stuck and need some help, check out the following hints:
...
```js
 ...
 ##### Be sure to uncomment the function calls below your `penTheAnimals` function to test it



## Part 3: More Refactoring
...
## Part 4: Single-Responsibility Principle
...
## Part 5: Use the return from one function to determine how another function handles data
...
##### Uncomment the penTheAnimals4 function invocations if you want see that this newest refactoring still draws our animals into their pens; don't forget to comment them back out again before moving on to part 6



## Part 6: Write two functions that return the arrays we need to draw
...
Your challenge in Part 6 is to write the code for `filterSocialAnimalsByType` and `filterSolitaryAnimals`. Use the notes as hints, and remember we need these functions to return arrays that can be sent into our draw function. 



## Part 7
#### Part 7 is the final part of this challenge! Whew! 
Let's refactor our code one final time so that we can call our `penTheAnimals` function without any arguments. We can get all the information we need by iterating over our `newAnimals` array. This final refactored function will:  
